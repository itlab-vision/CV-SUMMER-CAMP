...
// -video_name="C:\CV-SUMMER-CAMP\data\catdog.mp4" -start_frame="0" -frame_step="1" -detector_model="C:\CV-SUMMER-CAMP\data\mobilenet-ssd.caffemodel" -detector_weights="C:\CV-SUMMER-CAMP\data\mobilenet-ssd.prototxt" -labelpath="C:\CV-SUMMER-CAMP\data\labels.txt"
...
int main(int argc, char** argv) {
	...
	string desired_class= parser.get<String>("desired_class_id");
	String label_path = parser.get<String>("label_path");
	desired_class += ' ';
	vector<int> desired_class_id;
	int tmp = 0;
	int i = 1;
	for (auto x : desired_class)
	{
		...
		{
			desired_class_id.push_back(tmp);
			tmp = 0;
			i = 1;
		}
	}
	...
	Scalar color[9] = { Scalar(255,0,0),	Scalar(0,0,255),	 Scalar(0,255,0),
					     Scalar(255,255,0),	Scalar(255,0,255),	 Scalar(0,255,255),
						 Scalar(155,200,90),Scalar(200,90,155),	Scalar(90,155,200),};
    ...
    for (;; )
    {
        ...
        TrackedObjects detections = detector.detect(frame, frame_counter,label_path, desired_class_id);
        ...
        // Drawing all detected objects on a frame by BLACK
        for (const auto &detection : detections) {
            cv::rectangle(frame, detection.rect, Scalar(0,0,0), 3);
        }

        // Drawing tracked detections  and print classname with ID and detection
        // confidence level.
        for (const auto &detection : tracker->trackedDetections()) {
            cv::rectangle(frame, detection.rect, color[detection.object_id%9], 1);
            std::string text =detection.cName +"("+ std::to_string(detection.object_id)+")" +
                " conf: " + std::to_string(detection.confidence);
            cv::putText(frame, text, detection.rect.tl()+Point(5,20), cv::FONT_ITALIC,
                0.6, color[detection.object_id%9], 2);
        }
