...
int main(int argc, const char** argv) {
  ...
  ifstream ifs(lp);
  int num_c = 0;
  while (getline(ifs, lab[num_c]) && num_c < 1000)num_c++;
  ...
  for (;; )
  {
	  ...
	  for (const auto &detection : tracker->trackedDetections()) {
		  ...
		  std::string text = (labels[detector.classId[i]]) +
			  " conf: " + std::to_string(detection.confidence);
		  cv::putText(frame, text, detection.rect.tl(), cv::FONT_HERSHEY_COMPLEX,
			  1.0, cv::Scalar(0, 0, 255), 1);
		  Rect crop = detection.rect;
		  croped_image = frame(crop);
		  Mat res = ds.Classify(croped_image);
		  double confidence;
		  Point classIdPoint;
		  minMaxLoc(res, 0, &confidence, 0, &classIdPoint);
		  
		  if (classIdPoint.x < 277 && classIdPoint.x > 150) {
			  cout << "Conf:" << confidence << endl << "Class:" << lab[classIdPoint.x] << endl;
			  if (find(was.begin(), was.end(), classIdPoint.x) == was.end()) {
					  was.push_back(classIdPoint.x);
					  string text = "../Dogs/" + lab[classIdPoint.x] + ".png";
