...
public:
    ...
    TrackedObjects detect(const cv::Mat& frame, int frame_idx)
    {
        ...
        for (int i = 0; i < detection_as_mat.rows; i++)
        {
			...
			if (find(desired_class_id.begin(), desired_class_id.end(), cur_class_id) != desired_class_id.end())
			{
...
int main(int argc, char** argv) {
	...
	stringstream ss(desired_class_id);
	string item;
	while (getline(ss, item, ' ')) 
	{
		idClass.push_back(stoi(item));
    ...
    DnnObjectDetector detector(detector_model, detector_weights, idClass);
    ...
    for (;; )
    {
        ...
        for (const auto &detection : tracker->trackedDetections()) {
            cv::rectangle(frame, detection.rect, color[detection.classid], 3);
            std::string text = std::to_string(detection.object_id) + " : " + label[detection.classid] +
