...
// -video_name="P:/GitHub/CV-SUMMER-CAMP/data/iron.mp4" -start_frame="0" -frame_step="1" -detector_model="P:/GitHub/CV-SUMMER-CAMP/data/mobilenet-ssd.caffemodel" -detector_weights="P:/GitHub/CV-SUMMER-CAMP/data/mobilenet-ssd.prototxt"
...
public:
	...
	TrackedObjects detect(const cv::Mat& frame, int frame_idx)
	{
		...
		for (int i = 0; i < detection_as_mat.rows; i++)
		{
			...
			bool isFound = false;
			int classId = -1;
			for (auto &id : desired_class_id)
			{
				if (id >= 0 && cur_class_id == id)
				{
					classId = id;
					isFound = true;
					break;
			...
			TrackedObject cur_obj(cur_rect, cur_confidence, frame_idx, -1, classId);
...
int main(int argc, char** argv) {
	...
	cv::String desired_class_id = parser.get<cv::String>("desired_class_id");
	...
	DnnObjectDetector detector(detector_model, detector_weights, ids);
	...
	for (;; )
	{
		...
		for (const auto &detection : tracker->trackedDetections()) {
			...
			std::string text = "classId: " + std::to_string(detection.class_id) + ", objId: " + std::to_string(detection.object_id);// +" conf: " + std::to_string(detection.confidence);
