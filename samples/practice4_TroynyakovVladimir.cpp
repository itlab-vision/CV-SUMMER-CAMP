...
"{desired_class_id | | The desired class that should be tracked }"
};
...
public:
	...
	TrackedObjects detect(const cv::Mat& frame, int frame_idx)
	{
		...
		for (int i = 0; i < detection_as_mat.rows; i++)
		{
			...
			if (cur_rect.empty())
				continue;
			if (std::find(desired_class_id.begin(), desired_class_id.end(), cur_class_id) != desired_class_id.end()) {
...
int main(int argc, char** argv) {
	...
	std::stringstream desired_class_id(parser.get<string>("desired_class_id")); 
	...
	Scalar color[21] = { (188, 143, 143), (255, 250, 240), (128, 0, 128), (0, 0, 128),
						 (176, 224, 230),(255, 0, 255), (128, 0, 0),  (128, 128, 0), 
						 (0, 255, 255), (210, 105, 30), (255, 218, 185),(0, 255, 127),  
						 (255, 255, 0), (0, 255, 0), (250, 128, 114), (32, 178, 170),
						 (255, 0, 0), (244, 164, 96),  (127, 255, 212), (220, 20, 60), 
						 (85, 107, 47) };
	//open the capture
	VideoCapture cap;
	cap.open(video_name);
	...
	string line;
	while (std::getline(desired_class_id, line, ' ')) {
		class_id.push_back(stoi(line));
	}
	...
	DnnObjectDetector detector(detector_model, detector_weights, class_id);
	...
	for (;; )
	{
		...
		for (const auto &detection : tracker->trackedDetections()) {
			...
			std::string text = label[detection.class_id] +
				" conf: " + std::to_string(detection.confidence);
			cv::putText(frame, text, detection.rect.tl(), cv::FONT_HERSHEY_SIMPLEX,
				0.5, color[detection.object_id], 2);
		}
