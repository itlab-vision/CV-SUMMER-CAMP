...
{   "{ video_name        |  | video name                                }"
    ...
    "{ desired_class_ids |-1| The desired class that should be tracked  }"
...
int main(int argc, char** argv) {
    ...
    for (;; )
    {
        ...
        for (const auto &detection : tracker->trackedDetections()) {
            cv::rectangle(frame, detection.rect, cv::Scalar(12*detection.class_id,  255, 255-12*detection.class_id), 2);
            
            std::string text = classes[detection.class_id] + " "
                                + std::to_string(detection.object_id)
                                + " conf: " + format("%.3f", detection.confidence);
