    ...
    for (;; )
    {
       ...
       // frame = tracker->drawActiveTracks(frame);
        ...
        for (const auto &detection : tracker->trackedDetections()) {
            ...
            cv::putText(frame, text, detection.rect.tl(), cv::FONT_HERSHEY_TRIPLEX,
                1.0, cv::Scalar(0, 255, 0), 3);



            // imwrite(std::to_string(detection.object_id) + ".jpg", frame);

            Mat img = frame;
            Mat tmp = img(detection.rect);


            String imgName(std::to_string(detection.object_id) + ".jpg");
            ...
            if (classId >= 150 && classId <= 277 && confidence >= 0.6) {
