...
"{desired_class_id |8| The desired class that should be tracked }"
};
    ...
    TrackedObjects detect(const cv::Mat& frame, int frame_idx)
    {
        ...
        for (int i = 0; i < detection_as_mat.rows; i++)
        {
            ...
            if ((desired_class_id >= 0) && (cur_class_id != desired_class_id))
                continue;
			if ((cur_class_id != 3) && (cur_class_id != 8) && (cur_class_id != 10) && (cur_class_id != 12) && (cur_class_id != 12) && (cur_class_id != 15) && (cur_class_id != 17))
				continue;
...
int main(int argc, char** argv) {
    ...
    for (;; )
    {
        ...
        for (const auto &detection : detections) {
            cv::rectangle(frame, detection.rect, cv::Scalar(255-(detection.class_id*12), detection.class_id*12, detection.class_id*12), 3);
        }

        // Drawing tracked detections only by RED color and print ID and detection
        // confidence level.
        for (const auto &detection : tracker->trackedDetections()) {
            cv::rectangle(frame, detection.rect, cv::Scalar(detection.class_id*12, detection.class_id*12, 255-(detection.class_id * 12)), 3);
            std::string text = std::to_string(detection.class_id) +
               " conf: " + std::to_string(detection.confidence);
            cv::putText(frame, text, detection.rect.tl(), cv::FONT_HERSHEY_COMPLEX,
                1.0, cv::Scalar(detection.class_id * 12, detection.class_id * 12, 255 - (detection.class_id * 12)), 2, 4);
        }
