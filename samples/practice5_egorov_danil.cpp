...
"{label_path       |0| path to class labels             }"
...
public:
    ...
    TrackedObjects detect(const cv::Mat& frame, int frame_idx)
    {
        ...
        for (int i = 0; i < detection_as_mat.rows; i++)
        {
			...
			if (desired_class_id.back() >= 0)
			{
				bool has_cur_class_id = false;
				for (auto x : desired_class_id)
				{
					if (x == cur_class_id) 
						has_cur_class_id = true;
				}
				if(!has_cur_class_id) continue;
			}
...
int main(int argc, char** argv) {
	...
	string desired_classes_id_string = parser.get<string>("desired_class_id");
	...
	stringstream ss(desired_classes_id_string);
	vector<int> desired_classes_id;
	copy(istream_iterator<int>(ss), {}, back_inserter(desired_classes_id));
	copy(begin(desired_classes_id), end(desired_classes_id), ostream_iterator<int>(cout, " "));
    ...
    DnnObjectDetector detector(detector_model, detector_weights, desired_classes_id);
	DnnClassificator classificator(model_path, config_path, label_path, width, heigth, mean, swap);

	std::ofstream out;
	out.open("C:\\Users\\temp2019\\Desktop\\classified\\classified.txt");
    ...
    for (;; )
    {
        ...
        for (const auto &detection : tracker->trackedDetections()) {
			cv::Scalar color((detection.object_id * 10)%255, (detection.object_id * 10) % 255, (detection.object_id * 10) % 255);
			...
			if (lastDetected < detection.object_id) {
				Mat imgClassify = frame(detection.rect);
				Mat prob = classificator.Classify(imgClassify);

				Point classIdPoint;
				double confidence;
				minMaxLoc(prob.reshape(1, 1), 0, 0, &classIdPoint);

				if (out.is_open()) out << classIdPoint.x << std::endl;
				stringstream ss;
				ss << detection.object_id;
				string strClassifiedImg = "C:/Users/temp2019/Desktop/classified/" + ss.str() + ".jpg";
				imwrite(strClassifiedImg, imgClassify);

				lastDetected = detection.object_id;
			}
