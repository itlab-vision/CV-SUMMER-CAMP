...
С решения `CV_SUMMER_CAMP.sln`. В терминал-классах
  ...
  1. Можно запустить шаблонное приложение `practice1.exe`. Возможное сообщение
     ...
     к бинарным файлам проекта. Решение 2: добавить путь `С:\Intel\computer_vision_sdk\opencv\bin`
...
## Добавление собственных параметров в приложение

Список параметов для приложения задается в строке строке `cmdOptions`, дополнительные параметры вам нужно добавить самостоятельно.
...
## Детальная инструкция по выполнению работы
  1. С помощью инструкций из раздела `Общие инструкции по работе с Git` сойдайте форк [репозитория][origin] и создайте локальный репозиторий у себя на машине. 
  1. Разработать объявление наследника `GrayFilter` класса `Filter`. Реализовать для него функцию `ProcessImage`, которая которая будет возвращать изображение в оттенках серого. Для этого нужно использовать функцию [`cvtColor`][cvtcolor]. 
  1. Разработать объявление наследника `ResizeFilter` класса `Filter`. Реализовать для него конструктор, принимающий два параметра - новую ширину и высоту изображения и функцию `ProcessImage`, которая которая будет возвращать изображение нового размера. Для этого нужно использовать функцию [`cvResize`][cvresize].
